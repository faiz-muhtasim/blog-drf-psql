...
class CommentListCreateView(APIView):
    ...
    def post(self, request):
        try:
            ...
            comment = Comments.objects.create_comment(serializer.validated_data, user=request.user)  # 👈 pass user
...
class CommentRetrieveUpdateDeleteView(APIView):
    ...
    def put(self, request, pk):
        try:
            comment = Comments.objects.get_comment_by_id(pk)                  # 👈 fetch first
            if not comment:
                return Response(error_response(message="Comment not found", code=404), status=status.HTTP_404_NOT_FOUND)
            if comment.user != request.user:                                   # 👈 ownership check
                return Response(error_response(message="You are not allowed to edit this comment", code=403), status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, pk):
        try:
            ...
            if not comment:
                return Response(error_response(message="Comment not found", code=404), status=status.HTTP_404_NOT_FOUND)
            if comment.user != request.user:                                   # 👈 ownership check
                return Response(error_response(message="You are not allowed to delete this comment", code=403), status=status.HTTP_403_FORBIDDEN)
